...
public class ReaderModiAndDelIFrame extends JInternalFrame {
	...
	private String[] array=new String[]{"身份证","学生证","军人证","二货证"};
	...
	public ReaderModiAndDelIFrame() {
		...
		Object[][] results=getFileStates(ReaderDao.selectReader());
	...
	private final class DelButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			int i=ReaderDao.DelReader(ISBN.getText().trim());
			if(i==1){
	...
	class ModiButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			int i=ReaderDao.UpdateReader(readername.getText().trim(), sex, age.getText().trim(), zjnumber.getText().trim(), Date.valueOf(date.getText().trim()), maxnumber.getText().trim(), tel.getText().trim(), Double.valueOf(keepmoney.getText().trim()), zj, zy.getText().trim(), Date.valueOf(bztime.getText().trim()), ISBN.getText().trim());
			//数据库中无id属性，所以对应的所有与id有关的内容都删除就行
