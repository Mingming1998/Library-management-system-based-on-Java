...
public class BookTypeModiAndDelIFrame extends JInternalFrame {
	...
	public BookTypeModiAndDelIFrame() {
		...
		Object[][] results=getFileStates(BookTypeDao.selectBookCategory());
		...
		//从数据库中取出图书类别
		List list=BookTypeDao.selectBookCategory();
		for(int i=0;i<list.size();i++){
			BookType booktype=(BookType)list.get(i);
			Item item=new Item();
	...
	class ButtonAddListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			int i=BookTypeDao.UpdatebookType(BookTypeId.getText().trim(),selectedItem.toString(), days.getText().trim(),fk.getText().trim());
