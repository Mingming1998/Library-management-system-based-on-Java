...
public class BookBorrowIFrame extends JInternalFrame {
	...
	public Date getBackTime() {	//取还书时间
		String days = "0";
		List list2 = BookBorrowBackDao.selectBookCategory(bookType.getText().trim());
...
//		});//关闭窗口时候引发的事件
		...
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				if (readerISBN.getText().trim().length()!=0
					...
					String ISBNs = bookISBN.getText().trim();//得到文本并除去空格
					List list = BookBorrowBackDao.selectBookInfo(ISBNs);
					...
					String readerISBNs = readerISBN.getText().trim();//得到文本并除去空格
					List list5 = BookBorrowBackDao.selectReader(readerISBNs);// 此读者是否在tb_reader表中
					List list4 = BookBorrowBackDao.selectBookInfo(ISBNs);// 此书是否在tb_bookInfo表中
	...
	class ISBNListenerlostFocus extends KeyAdapter {
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				List list = BookBorrowBackDao.selectReader(ISBNs);
	...
	class BorrowActionListener implements ActionListener { 
		public void actionPerformed(final ActionEvent e) {
			...
			int i=BookBorrowBackDao.InsertBookBorrow(bookISBNs, readerISBNs, operatorId, 
					java.sql.Timestamp.valueOf(borrowDate), java.sql.Timestamp.valueOf(backDate));
			if(i==1){
